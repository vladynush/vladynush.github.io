...
export type NumberedValue<T> = { value: T; number: number };
export type Customer = { id: string; name: string; age: number; isSubscribed: boolean };
export type TransformedCustomer = Record<string, Omit<Customer, 'id'>>;

type MapperInput<T> = T[];
export const getNumberedArray = <T>(arr: MapperInput<T>): NumberedValue<T>[] =>
  arr.map((value, number) => ({ value, number }));
export const toStringArray = (arr: NumberedValue<string>[]): string[] =>
  arr.map(({ value, number }): string => `${value}_${number}`);

export const transformCustomers = (customers: Customer[]): TransformedCustomer => {
  return customers.reduce((acc, customer): TransformedCustomer => {
